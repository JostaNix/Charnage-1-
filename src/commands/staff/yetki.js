...
module.exports = {
    ...
    async execute(client, message, args, Embed) {
        ...
        if(['ekle', 'oluştur', 'add'].some(arg => args[0].toLocaleLowerCase() == arg)) {
            ...
            if(!user && !role && !channel) return message.channel.error(message, Embed.setDescription(`${mark ? mark : ``}  Doğru kullanım : \`${Prefix}yetki ver ${name} [<@Noâ/ID> / <@Rol/ID> / <#Kanal/ID>]\``), { timeout: 10000, react: true });
            ...
            if(!args[2]) return message.channel.error(message, Embed.setDescription(`Bir üye, rol veya ses kanalı belirtmelisin!`), { timeout: 8000, react: true });
            if(!user && !role && !channel) return message.channel.error(message, Embed.setDescription(`${mark ? mark : ``}  Doğru kullanım : \`${Prefix}yetki al ${name} [<@Noâ/ID> / <@Rol/ID> / <#Kanal/ID>]\``), { timeout: 10000, react: true });
